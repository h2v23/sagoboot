...
class Options implements \SagoBoot\Singleton
{
	 ...
	 * @var TableFormatter
	 */
	protected $table_formatter;

	/**
	 * Constructor
	 *
	 * @param Colors $colors optional configured color object
	 * @param TableFormatter $table_formatter
	 */
	public function __construct(
		\SagoBoot\Cli\Colors $colors,
		\SagoBoot\Cli\TableFormatter $table_formatter
	)
	{
		$this->colors = $colors;
		$this->table_formatter = $table_formatter;
		$this->setup = array(
			'' => array(
				'opts' => array(),
				'args' => array(),
	...
	public function help()
	{
		...
		foreach ($this->setup as $command => $config) {
			...
			if ($this->setup[$command]['help']) {
				$text .= "\n";
				$text .= $this->table_formatter->format(
					array($mv, '*'),
